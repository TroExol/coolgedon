...
describe('familiars 7', () => {
  ...
  beforeEach(() => {
    ...
    familiar7 = testHelper.createMockCard(room, cardMap[ECardTypes.familiars][7]);
    testHelper.giveCardToPlayer(familiar7, otherPlayer);
  });

  test('Разыгрывается и перенаправляет атаку', async () => {
    await familiar7.playGuard({ cardAttack, attacker: activePlayer, damage: 5 });

    expect(familiar7.played).toBeFalsy();
  ...
  test('Разыгрывается и не перенаправляет атаку без атакующего', async () => {
    await familiar7.playGuard({ cardAttack, damage: 5 });
