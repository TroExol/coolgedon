...
describe('familiars 4', () => {
  ...
  beforeEach(() => {
    ...
    testHelper.giveCardToPlayer(familiar4, otherPlayer);
  });

  test('Разыгрывается и перенаправляет атаку', async () => {
    await familiar4.playGuard({ cardAttack, attacker: activePlayer, damage: 5 });
  ...
  test('Разыгрывается и не перенаправляет атаку без атакующего', async () => {
    await familiar4.playGuard({ cardAttack, damage: 5 });
