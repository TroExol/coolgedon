...
describe('familiars 10', () => {
  ...
  beforeEach(() => {
    ...
    testHelper.giveCardToPlayer(familiar10, otherPlayer);
  });

  test('Разыгрывается и перенаправляет атаку', async () => {
    await familiar10.playGuard({ cardAttack, attacker: activePlayer, damage: 5 });
  ...
  test('Разыгрывается и не перенаправляет атаку без атакующего', async () => {
    await familiar10.playGuard({ cardAttack, damage: 5 });
