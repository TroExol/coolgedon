...
describe('familiars 9', () => {
  ...
  beforeEach(() => {
    ...
    testHelper.giveCardToPlayer(familiar9, otherPlayer);
  });

  test('Разыгрывается и перенаправляет атаку', async () => {
    await familiar9.playGuard({ cardAttack, attacker: activePlayer, damage: 5 });
  ...
  test('Разыгрывается и не перенаправляет атаку без атакующего', async () => {
    await familiar9.playGuard({ cardAttack, damage: 5 });
