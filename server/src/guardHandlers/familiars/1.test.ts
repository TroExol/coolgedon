...
describe('familiars 1', () => {
  ...
  beforeEach(() => {
    ...
    testHelper.giveCardToPlayer(familiar1, otherPlayer);
  });

  test('Разыгрывается и перенаправляет атаку', async () => {
    await familiar1.playGuard({ cardAttack, attacker: activePlayer, damage: 5 });

    expect(familiar1.played).toBeFalsy();
  ...
  test('Разыгрывается и не перенаправляет атаку без атакующего', async () => {
    await familiar1.playGuard({ cardAttack, damage: 5 });
