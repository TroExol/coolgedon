...
describe('familiars 6', () => {
  ...
  beforeEach(() => {
    ...
    familiar6 = testHelper.createMockCard(room, cardMap[ECardTypes.familiars][6]);
    testHelper.giveCardToPlayer(familiar6, otherPlayer);
  });

  test('Разыгрывается и перенаправляет атаку', async () => {
    await familiar6.playGuard({ cardAttack, attacker: activePlayer, damage: 5 });

    expect(familiar6.played).toBeFalsy();
  ...
  test('Разыгрывается и не перенаправляет атаку без атакующего', async () => {
    await familiar6.playGuard({ cardAttack, damage: 5 });
