...
describe('familiars 2', () => {
  ...
  beforeEach(() => {
    ...
    familiar2 = testHelper.createMockCard(room, cardMap[ECardTypes.familiars][2]);
    testHelper.giveCardToPlayer(familiar2, otherPlayer);
  });

  test('Разыгрывается и перенаправляет атаку', async () => {
    await familiar2.playGuard({ cardAttack, attacker: activePlayer, damage: 5 });

    expect(familiar2.played).toBeFalsy();
  ...
  test('Разыгрывается и не перенаправляет атаку без атакующего', async () => {
    await familiar2.playGuard({ cardAttack, damage: 5 });
