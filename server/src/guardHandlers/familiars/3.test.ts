...
describe('familiars 3', () => {
  ...
  beforeEach(() => {
    ...
    testHelper.giveCardToPlayer(familiar3, otherPlayer);
  });

  test('Разыгрывается и перенаправляет атаку', async () => {
    await familiar3.playGuard({ cardAttack, attacker: activePlayer, damage: 5 });

    expect(familiar3.played).toBeFalsy();
  ...
  test('Разыгрывается и не перенаправляет атаку без атакующего', async () => {
    await familiar3.playGuard({ cardAttack, damage: 5 });
