...
describe('treasures 5', () => {
  ...
  beforeEach(() => {
    ...
    treasure5 = testHelper.createMockCard(room, cardMap[ECardTypes.treasures][5]);
    testHelper.giveCardToPlayer(treasure5, otherPlayer);
  });

  test('Разыгрывается и наносит урон атакующему', async () => {
    await treasure5.playGuard({ cardAttack, attacker: activePlayer, damage: 5 });

    expect(treasure5.played).toBeFalsy();
  ...
  test('Разыгрывается и не наносит урон без атакующего', async () => {
    await treasure5.playGuard({ cardAttack, damage: 5 });
